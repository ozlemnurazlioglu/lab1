...
cursorObject.execute("create database ıf it is not exist marvel")
cursorObject.close()
...
def add_to_database(id_entry, movie_entry, date_entry, phase_entry):
    ...
    my_sql_insert = """INSERT INTO MarvelInfo (ID,Movie,DateInfo,Mcu_Phase)
                                                          VALUES (%s,%s,%s,%s)"""
    data = (id_value, movie_value, date_value, phase_value)
    cursorObject2.execute(my_sql_insert, data)
    connection.commit()
...
box_frame = ttk.Frame(root, padding="20")
